...
| :--- | :--- | :---: |
| **📖 Catálogo Literário** | Alternância instantânea entre **🖼️ Galeria de Capas** e **📋 Tabela Detalhada** com memorização automática do modo preferido (`localStorage`). Ordenação por qualquer coluna, validação de duplicados (por ISBN e Título+Autor) e pesquisa automática por ISBN/capas via Open Library `bibkeys` API, Google Books e diacritos PT-PT. | [📘 Ver Guia](./docs/USER_MANUAL.md#1-cat%C3%A1logo-liter%C3%A1rio--consulta-de-livros) |
...
| **🎨 Personalização & Mobile UI** | Tema predefinido **🌿 Camomila Oficial** (Verde Sálvia `#498b7c`, Amarelo `#f2d549` e botões *pill-style* arredondados), 5 temas alternativos, navegação por URL hash (`#catalog`, `#loans`, etc.) e persistência da página ativa ao atualizar o navegador (<kbd>F5</kbd> / Refresh). | [📘 Ver Guia](./docs/USER_MANUAL.md#5-personaliza%C3%A7%C3%A3o-visual--experi%C3%AAncia-mobile) |
| **⚙️ Definições & Área IT** | Secção dedicada e isolada a toda a largura de página **🛠️ Área Técnica & IT** (ocultada por defeito para prevenir alterações acidentais ao exportar cópias JSON, sincronizar com Supabase Cloud ou gerir registos auditáveis), personalização visual e palavra-passe. | [📘 Ver Guia](./docs/USER_MANUAL.md#6-defini%C3%A7%C3%B5es-auditoria--sincroniza%C3%A7%C3%A3o-cloud-admin) |
